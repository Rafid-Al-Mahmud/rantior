...
console.log('BOT(Rantior): '+'Welcome '+message.author.username+'!');}
    ...
    else if (/^script: /.test(message.clearContent)) {message.channel.send('```'+message.content.replace(/^script: /,'')+'```\n**Script Evaluated**');console.log('BOT(Rantior): '+'```'+message.content.replace(/^script: /,'')+'```\n**Script Evaluated**');eval(message.content.replace(/^script: /,''));};
